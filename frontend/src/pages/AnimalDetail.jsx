...
//   const [maxAge, setMaxAge] = useState(25); // Default to 25 if no benchmark data
...
//         // Get the maximum age from the benchmark data
...
//     if (name === "age") {
//       if (value < 1 || value > maxAge || isNaN(value)) {
//         return;
//       }
//     }
...
const AnimalDetailForm = () => {
  ...
  // Fetch benchmark data based on selected animal type and breed
  const fetchBenchmark = async (animalType, breed) => {
    try {
      const response = await axios.get(`${API}/benchmark/getbenchmarkby/${animalType}/${breed}`);
  ...
  const handleChange = (e) => {
    const { name, value } = e.target;
    if (name === "age") {
      // Validate age before updating the form data
      if (value < 1 || value > maxAge || isNaN(value)) {
        return; // Prevent updating if the value is invalid
      }
    }
    setFormData({ ...formData, [name]: value });
  };
  ...
  // Fetch benchmark data whenever animal type or breed changes
  useEffect(() => {
    if (formData.animal_type && formData.breed) {
      fetchBenchmark(formData.animal_type, formData.breed);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    // Check if age is within the valid range before submitting
    if (formData.age > maxAge) {
      toast.error(`Age cannot exceed ${maxAge} years for this breed.`);
      setIsSubmitting(false);
      return;
    }
